...
const Paises = ({data = null, addCountry}) => {
    const [countrySelected, setCountrySelected] = useState(data == null ? "Chile" : data.split("-")[1].trim())
            ...
            {Countries[0] != undefined ? (
                <div className="modal-filter__list scrollable-custom">
                    {Countries.map((Country, index) => (
                        <div>
                            <div
                                className={`${quitarDiacriticos(Country.title) == countrySelected ? "result-selected" : "result" } result--pinned dashed dashed`}
